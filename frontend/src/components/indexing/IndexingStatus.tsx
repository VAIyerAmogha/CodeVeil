  ...
  if (state === 'complete') {
    return (
      <div className="w-full max-w-lg bg-green-500/10 border border-green-500/20 rounded-xl p-6 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-sm text-green-100/60">Redirecting to knowledge hub...</p>
      </div>
    );
  }
  ...
  if (state === 'failed') {
    return (
      <div className="w-full max-w-lg bg-red-500/10 border border-red-500/20 rounded-xl p-6 flex flex-col items-center text-center">
        ...
        <p className="text-sm text-green-100/80 mb-6">{error || 'An unknown error occurred during indexing.'}</p>
        <button
          onClick={onReset}
          className="bg-green-900/20 border-green-500/20 hover:bg-green-800/40 text-green-50 font-medium px-6 py-2 rounded-lg transition-colors text-sm"
        >
          Try Again
        </button>
      </div>
